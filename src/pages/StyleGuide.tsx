...
import { RadiusSwatch } from '../components/styleguide/RadiusSwatch';
import { ShadowSwatch } from '../components/styleguide/ShadowSwatch';
import { TypeSpecimen } from '../components/styleguide/TypeSpecimen';
import { colors, radius, typeScale, orphanLineHeights, fontWeights, shadowScale, insetShadows } from '../tokens/generated';
    ...
    <div className="flex flex-col gap-md">
      {BUTTON_SIZES.map((size) => (
        <div key={size} className="flex flex-wrap items-center gap-md">
          <span className="w-8 text-caption text-text-secondary">{size}</span>
      ...
      <p className="text-caption text-text-secondary">Hover / click the buttons above to preview live hover &amp; active states.</p>
    </div>
  );
}
...
export default function StyleGuide() {
  ...
  const primaryColors = Object.entries(colors).filter(([k]) => k.startsWith('primary-'));
  const grayColors = Object.entries(colors).filter(([k]) => k.startsWith('gray-'));
  const errorColors = Object.entries(colors).filter(([k]) => k.startsWith('error-'));
  const warningColors = Object.entries(colors).filter(([k]) => k.startsWith('warning-'));
  const successColors = Object.entries(colors).filter(([k]) => k.startsWith('success-'));
  const infoColors = Object.entries(colors).filter(([k]) => k.startsWith('info-'));
  const backgroundKeys = ['card', 'well', 'nested-card', 'bg-layout', 'bg-container-1', 'bg-group-card', 'secondary-bg', 'secondary-2-bg'];
  const backgroundColors = backgroundKeys.map((k) => [k, colors[k as keyof typeof colors]] as const);
  ...
  return (
    <div className="style-guide">
      ...
      <div className="guide-layout">
        ...
        <main className="guide-main min-w-0 flex-1" id="top">
          <header className="guide-hero">
            <div className="guide-hero__copy">
              <div className="guide-kicker"><span>●</span> Interface library</div>
              <h1>Build with clarity.<br /><em>Ship with consistency.</em></h1>
              <p>A living reference for ThinkFirm’s components, interaction states, and design foundations—ready to inspect, test, and build from.</p>
              <div className="guide-hero__actions">
                <a href="#buttons">Explore components <span>↓</span></a>
                <span>Powered by <code>tokens.json</code></span>
              </div>
            </div>
            <div className="guide-hero__stats" aria-label="Style guide summary">
              <div><strong>08</strong><span>Component groups</span></div>
              <div><strong>{Object.keys(colors).length}</strong><span>Color tokens</span></div>
              <div><strong>100%</strong><span>Inspectable</span></div>
            </div>
          </header>

        <Section id="buttons" title="Buttons" description="Primary, stroke (brand + gray) and ghost variants across sm/md/lg, plus disabled and loading states.">
        ...
        <Section id="button-groups" title="Button Groups & Split Button" description="Joined button rows, and a primary action with an attached dropdown of secondary actions.">
        ...
        <Section id="checkboxes" title="Checkboxes & Radio" description="Unchecked, checked, indeterminate and disabled states, each in three sizes.">
          ...
          <SubSection title="Checkbox list">
            <div className="flex flex-col gap-sm rounded-md border border-gray-4 bg-card p-lg">
          ...
          <SubSection title="Radio — sizes">
            <div className="flex flex-wrap items-center gap-2xl">
              <Radio size="sm" name="radio-size-demo-sm" label="Small" checked readOnly />
              <Radio size="md" name="radio-size-demo-md" label="Medium (default)" checked readOnly />
              <Radio size="lg" name="radio-size-demo-lg" label="Large" checked readOnly />
            </div>
          </SubSection>
          <SubSection title="Radio — group">
            <div className="flex flex-col gap-sm rounded-md border border-gray-4 bg-card p-lg">
              ...
              <Radio name="radio-plan" label="Enterprise (disabled)" disabled checked={false} onChange={() => {}} />
            </div>
          </SubSection>
        </Section>

        <Section id="switches" title="Switches" description="On/off and disabled variants, in three sizes — track capped at radius-xs (4px), knob at radius-xxs (2px).">
        ...
        <Section id="inputs" title="Inputs & Selects" description="Text inputs, native selects and a searchable combobox — all capped at radius-xs (4px).">
          <SubSection title="Text input — sizes">
            <div className="flex max-w-[36rem] flex-col gap-lg">
          ...
          <SubSection title="Text input — states">
            <div className="grid max-w-[36rem] grid-cols-2 gap-lg">
          ...
          <SubSection title="Searchable combobox — variants">
            <div className="grid max-w-[36rem] grid-cols-1 gap-lg">
        ...
        <Section id="tabs" title="Tabs" description="Underline, segmented/pill, boxed/card and two icon-tab styles.">
        ...
        <Section id="modals" title="Modals" description="Standard modal (sm/md/lg), scrollable body, and confirmation modals in all four color variants.">
        ...
        <Section id="pills-badges" title="Pills & Badges" description="Compact status labels and count/dot indicators — capped at radius-xxs (2px), same as checkboxes and tag chips.">
          ...
          <SubSection title="Badges — count">
            <div className="flex flex-wrap items-center gap-xl">
              <span className="inline-flex items-center gap-xs text-body text-text-primary">Inbox <Badge>4</Badge></span>
              <span className="inline-flex items-center gap-xs text-body text-text-primary">Errors <Badge variant="error">12</Badge></span>
              <span className="inline-flex items-center gap-xs text-body text-text-primary">Drafts <Badge variant="neutral">99+</Badge></span>
            </div>
          </SubSection>
          <SubSection title="Badges — dot indicator">
            <div className="flex flex-wrap items-center gap-2xl">
              <span className="relative inline-flex">
                ...
                <BadgeDot className="absolute -right-xxs -top-xxs" />
              </span>
              <span className="relative inline-flex">
                <Avatar name="Elin Sorensen" size="md" />
                <BadgeDot variant="success" className="absolute -right-xxs -top-xxs" />
              </span>
            </div>
          </SubSection>
        </Section>

        <Section id="progress" title="Progress" description="Determinate and indeterminate progress bars, in two sizes and every semantic color.">
          <SubSection title="Sizes & values">
            <div className="flex max-w-[28rem] flex-col gap-lg">
        ...
        <Section id="cards" title="Cards" description="Header/body/footer container, capped at radius-xs (4px).">
          <SubSection title="Basic & with header actions">
            <div className="grid max-w-[42rem] grid-cols-2 gap-lg">
              <Card title="Basic card">
                <p className="text-body text-text-secondary">A card with just a title and body content — no footer or header actions.</p>
              </Card>
            ...
            <div className="grid max-w-[42rem] grid-cols-2 gap-lg">
              <Card>
                <div className="flex flex-col gap-xs">
                  <span className="text-caption text-text-secondary">Monthly revenue</span>
                  <span className="text-h4 font-semibold text-text-heading">$48,200</span>
                  <Pill variant="success" dot className="w-fit">+12.4% vs last month</Pill>
                </div>
              </Card>
              <Card title="Delete workspace" subtitle="This action cannot be undone" footer={<div className="flex justify-end gap-sm"><Button variant="stroke-gray" size="sm">Cancel</Button><Button variant="primary" size="sm">Delete</Button></div>}>
                <p className="text-body text-text-secondary">All projects, members and billing history will be permanently removed.</p>
              </Card>
            </div>
          </SubSection>
        </Section>

        <Section id="avatars" title="Avatars" description="Circle (exempt from the radius ceiling, same reasoning as the radio dot) or square (radius-xs/4px). Initials fill deterministically from the primary ramp.">
          <SubSection title="Sizes">
          ...
          <SubSection title="Shape — square (radius-xs)">
            <div className="flex flex-wrap items-end gap-lg">
        ...
        <Section id="table" title="Table" description="Header row, hover states and mixed cell content (avatar, pill, row actions) — outer wrapper capped at radius-xs (4px).">
          <SubSection title="People">
            <Table columns={PEOPLE_COLUMNS} data={PEOPLE} getRowKey={(row) => row.id} />
          </SubSection>
        </Section>

        <Section id="colors" title="Colors & Backgrounds" description="Every color group in tokens.json, labeled with token name and hex value.">
          <SubSection title="Primary ramp (0–10)">
            <div className="grid grid-cols-6 gap-md sm:grid-cols-11">
              {primaryColors.map(([name, hex]) => (
                <ColorSwatch key={name} name={name} hex={hex} />
              ))}
            </div>
          </SubSection>
          <SubSection title="Neutral-Gray (1–13)">
            <div className="grid grid-cols-6 gap-md sm:grid-cols-[repeat(13,minmax(0,1fr))]">
        ...
        <Section id="typography" title="Typography, Radius & Shadows" description="Type scale, radius scale and shadow scale as visual references.">
          <SubSection title="Type scale">
            ...
            {orphanLineHeights.length > 0 && (
              <p className="text-caption text-text-secondary">
                Line-height tokens with no matching size token: {orphanLineHeights.map((o) => `${o.token} (${o.value}px)`).join(', ')}
              </p>
            )}
          </SubSection>
            ...
            <div className="flex flex-col gap-sm">
              {Object.entries(fontWeights).map(([key, label]) => (
                <div key={key} className="flex items-center gap-lg">
                  <span className="w-32 text-caption text-text-secondary">{key}</span>
                  <span
                    className={
                      key.includes('bold') && key.includes('italic')
                        ? 'text-body font-bold italic'
                        : key.includes('semibold') && key.includes('italic')
                          ? 'text-body font-semibold italic'
                          : key.includes('medium') && key.includes('italic')
                            ? 'text-body font-medium italic'
                            : key.includes('italic')
                              ? 'text-body font-normal italic'
                              : key === 'bold'
                                ? 'text-body font-bold'
                                : key === 'semibold'
          ...
          <SubSection title="Radius scale">
            <div className="grid grid-cols-6 gap-lg sm:grid-cols-11">
              {Object.entries(radius).map(([name, px]) => (
                <RadiusSwatch key={name} name={name} px={px} />
              ))}
            </div>
          </SubSection>
          <SubSection title="Shadow scale (approximated — offset/blur not in tokens.json, only colors)">
            <div className="grid grid-cols-4 gap-lg sm:grid-cols-7">
              {shadowScale.map((s) => (
                <ShadowSwatch key={s.tier} name={s.tier} boxShadow={s.boxShadow} tokens={s.layers.map((l) => l.token)} />
              ))}
            </div>
          </SubSection>
          <SubSection title="Inset shadows (skeuomorphic)">
            <div className="grid grid-cols-4 gap-lg sm:grid-cols-7">
              {insetShadows.map((s) => (
                <div key={s.name} className="flex flex-col items-center gap-xs">
                  <div className="h-4xl w-4xl rounded-md bg-well" style={{ boxShadow: s.boxShadow }} />
                  <div className="text-caption text-text-primary">{s.name}</div>
                  <div className="text-caption text-text-secondary">{s.token}</div>
                </div>
              ))}
            </div>
          </SubSection>
